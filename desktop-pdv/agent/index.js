...
class ThorAgent {
  ...
  async finalizeSale({items,customerId=null,payments=[],discount=0,notes=''}){
    const cashOpenEventId=this.store.get('cash_open_event_id'); if(!cashOpenEventId) throw new Error('cash_not_open'); if(!Array.isArray(items)||!items.length) throw new Error('empty_cart');
    let subtotal=0; const resolved=[];
    for(const item of items){ const p=this.store.product(item.productId); if(!p||!p.active) throw new Error('product_not_found'); const qty=Number(item.quantity||0); if(qty<=0) throw new Error('invalid_quantity'); const price=this.resolvePrice(p,qty); const itemDiscount=Math.max(Number(item.discount||0),0); subtotal+=qty*price-itemDiscount; resolved.push({product_id:p.id,quantity:qty,unit_price:price,discount:itemDiscount,name:p.name,sku:p.sku,unit:p.unit}); }
    const saleDiscount=Math.max(Number(discount||0),0); const total=Math.max(subtotal-saleDiscount,0); const normalizedPayments=(payments||[]).map(p=>({method:p.method,amount:Number(p.amount||0),provider:p.provider||null,external_id:p.externalId||null,txid:p.txid||null,metadata:p.metadata||{}})); const paid=normalizedPayments.reduce((s,p)=>s+p.amount,0); if(paid>total+0.01) throw new Error('payment_exceeds_total');
    const payload={cash_open_event_id:cashOpenEventId,customer_id:customerId||null,items:resolved,payments:normalizedPayments,discount:saleDiscount,notes}; const event=this.event('sale_completed',payload);
    for(const i of resolved) this.store.adjustInventory(i.product_id,-i.quantity);
    const receipt={eventId:event.id,items:resolved,subtotal,discount:saleDiscount,total,payments:normalizedPayments,customerId,createdAt:new Date().toISOString(),context:JSON.parse(this.store.get('context','{}')||'{}')}; this.store.saveReceipt(event.id,total,receipt);
    return {ok:true,eventId:event.id,subtotal,total,paid,receipt};
  }
