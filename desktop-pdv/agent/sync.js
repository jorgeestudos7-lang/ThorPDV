class SyncEngine {
  constructor({ store, apiBase, tokenProvider, onState, appVersion='0.2.0' }) {
  ...
  async run(){ if(this.running||!this.tokenProvider()) return {ok:false,error:'not_enrolled'}; this.running=true; this.onState({syncing:true}); try{
      ...
      await this.request('/api/pdv/heartbeat',{appVersion:this.appVersion,capabilities:{offline:true,printing:true,serial:true,fiscalMenu:true,returns:true,pdf:true,configurableShortcuts:true},metrics:{queue:this.store.queueStats()}});
