...
function installCashClosing(ThorAgent) {
  ...
  ThorAgent.prototype._localCashSummary = function ({ onlyUnsynced = false } = {}) {
    ...
    for (const row of rows) {
      ...
      if (row.type === 'sale_return' && String(payload.refund_method || '') === 'cash' && (!payload.cash_open_event_id || String(payload.cash_open_event_id) === String(openId))) {
        movements.refund += money(payload.refund_amount);
      }
    }
    ...
    return {
      ...
      supply: movements.supply,
      withdrawal: movements.withdrawal,
      expense: movements.expense,
      refund: movements.refund,
  ...
  ThorAgent.prototype.cashClosingPreview = async function () {
    ...
    try {
      ...
      const result = {
        ...
        sales_count: money(server.sales_count) + money(unsynced.sales_count),
        sales_total: money(server.sales_total) + money(unsynced.sales_total),
        cash_payments: money(server.cash_payments) + extraCash,
        supply: money(server.supply) + money(unsynced.supply),
        withdrawal: money(server.withdrawal) + money(unsynced.withdrawal),
        expense: money(server.expense) + money(unsynced.expense),
        refund: money(server.refund) + money(unsynced.refund),
        pending_events: unsynced.pending_events,
        rejected_events: unsynced.rejected_events,
      };
  ...
  ThorAgent.prototype.closeCash = async function (payload = {}) {
    ...
    const result = await originalCloseCash.call(this, payload);
    ...
    const summary = {
      ...
      close_event_id: result.eventId,
    };
    if (result?.eventId) {
      const row = this.store.db.prepare('select payload from queue where id=?').get(result.eventId);
      const merged = { ...json(row?.payload, {}), reconciliation: summary };
      this.store.db.prepare('update queue set payload=?,updated_at=? where id=?').run(JSON.stringify(merged), closedAt, result.eventId);
    }
    this.store.set('last_cash_close_summary', JSON.stringify(summary));
    return { ...result, summary };
  };

  ThorAgent.prototype.lastCashCloseSummary = function () {
    return json(this.store.get('last_cash_close_summary', '{}'), null);
  };
