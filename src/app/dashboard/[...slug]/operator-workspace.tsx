...
export function OperatorWorkspace({initialUsers,profiles,branches}:{initialUsers:Row[];profiles:Row[];branches:Row[]}){
  ...
  const save=()=>startTransition(async()=>{
    ...
    reset();setMessage('Operador salvo. Comissão atualizada e disponível nos relatórios. Sincronize o ThorPDV Desktop para atualizar o usuário.');
  });
  ...
  return <div className="operator-admin-grid">
    ...
    <section className="operator-admin-card">
      ...
      <div className="operator-table-wrap"><table className="operator-table"><thead><tr><th>Nome</th><th>Perfil</th><th>Filial</th><th>Comissão</th><th>Status</th><th>Ações</th></tr></thead><tbody>{users.map(u=><tr key={text(u.id)}><td><strong>{text(u.name)}</strong><small>{text(u.email)||'—'}</small></td><td>{text(u.profile)||'Sem perfil'}</td><td>{text(u.branch)||'Todas'}</td><td>{Number(u.commission_percent||0).toLocaleString('pt-BR',{minimumFractionDigits:0,maximumFractionDigits:2})}%</td><td><span className={u.active===false?'op-inactive':'op-active'}>{u.active===false?'Inativo':'Ativo'}</span></td><td><div className="op-row-actions"><button onClick={()=>edit(u)}>Editar</button><button onClick={()=>quickPin(u)}>Definir PIN</button></div></td></tr>)}</tbody></table></div>
    </section>
