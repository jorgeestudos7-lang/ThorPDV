...
export function StockTransferClient({ products, branches, history }: { products: Row[]; branches: Row[]; history: Row[] }) {
  ...
  const [rows, setRows] = useState(history.filter((r) => String(r.movement_type).startsWith('transfer_')));
  ...
  return <section className="erp-module-card">
    ...
    <div className="erp-table-scroll"><table className="erp-data-table"><thead><tr><th>Data</th><th>Produto</th><th>Tipo</th><th>Quantidade</th><th>Filial</th><th>Observação</th></tr></thead><tbody>{rows.length===0?<tr><td className="erp-empty" colSpan={6}>Nenhuma transferência registrada.</td></tr>:rows.map((r,i)=><tr key={String(r.id??i)}><td>{datetime(r.created_at)}</td><td>{String(r.product??'')}</td><td><span className="erp-pill">{String(r.movement_type)}</span></td><td>{String(r.quantity??0)}</td><td>{String(r.branch??'')}</td><td>{String(r.notes??'—')}</td></tr>)}</tbody></table></div>
  </section>;
}
...
export function PriceTablesClient({ tables, products, copyMode=false }: { tables: Row[]; products: Row[]; copyMode?: boolean }) {
  ...
  return <div className="erp-advanced-grid">
    ...
    <section className="erp-module-card"><div className="erp-table-scroll"><table className="erp-data-table"><thead><tr><th>Código</th><th>Produto</th><th>Preço base</th><th>Preço tabela</th><th>Diferença</th></tr></thead><tbody>{(detail.items??[]).length===0?<tr><td className="erp-empty" colSpan={5}>A tabela ainda não possui preços específicos.</td></tr>:(detail.items??[]).map((r,i)=><tr key={String(r.product_id??i)}><td>{String(r.sku??'—')}</td><td>{String(r.product??'')}</td><td>{money(r.base_price)}</td><td><strong>{money(r.price)}</strong></td><td>{money(Number(r.price??0)-Number(r.base_price??0))}</td></tr>)}</tbody></table></div></section>
  </div>;
}

type InventoryDetail = { inventory?: Row; items?: Row[] };
export function InventoryClient({ inventories }: { inventories: Row[] }) {
  const [rows,setRows]=useState(inventories);
...
export function ReportsClient({ type, branches, initial }: { type:'sales'|'finance'|'stock'; branches:Row[]; initial:Row[] }) {
  ...
  const cols=reportColumns[type];const total=rows.reduce((s,r)=>s+Number(r.revenue??r.total??r.stock_value??0),0);
  async function generate(){const r=await erpReport(type,start,end,branch);if(r.ok){setRows(r.data);setMessage(`Relatório atualizado: ${r.data.length} linha(s).`);}else setMessage(String(r.error??'Falha ao gerar relatório'));}
